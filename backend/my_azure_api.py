...
_url = 'https://westcentralus.api.cognitive.microsoft.com/vision/vision/v3.2-preview.1/read/analyzeResults/'
_key = "b44b5e609b15425884c74444cf237ede"
_maxNumRetries = 10
...
def processRequest(json, data, headers, params):
    ...
    while True:
        response = requests.post(
            _url, json=json, data=data, headers=headers, params=params)
...
def getOCRTextResult(operationLocation, headers):
    ...
    while True:
        response = requests.get(
            operationLocation, json=None, data=None, headers=headers, params=None)
        if response.status_code == 429:
            print("Message: %s" % (response.json()))
            if retries <= _maxNumRetries:
                time.sleep(1)
...
def text_from_image(img):
    ...
    if result is not None and result['status'] == 'Succeeded':
        ...
        img = cv2.cvtColor(cv2.imdecode(
            data8uint, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
